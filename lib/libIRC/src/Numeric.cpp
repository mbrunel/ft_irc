...
namespace Reply
{
	...
	{
		return (Numeric(RPL_LIST, channel + ' ' + Utils::to_string(visible) + " :" + topic));
	}
	...
	const Numeric time()
	{
		return (Numeric(RPL_TIME, Utils::to_date(::time(NULL), "%x - %I:%M:%S")));
	}

	const Numeric lusersclient(size_t nb)
	{
		return (Numeric(RPL_LUSERCLIENT, ":There are " + Utils::to_string(nb) + " users on 1 server"));	
	}

	const Numeric lusersop(size_t nb)
	{
		return (Numeric(RPL_LUSEROP, Utils::to_string(nb) + " :operator(s) online"));
	}

	const Numeric lusersunknown(size_t nb)
	{
		return (Numeric(RPL_LUSERUNKNOWN, Utils::to_string(nb) + " :unknown connection(s)"));
	}

	const Numeric luserschannels(size_t nb)
	{
		return (Numeric(RPL_LUSERCHANNELS, Utils::to_string(nb) + " :channels formed"));
	}
